...
pub async fn apply_state_changes(
    ...
    for mutation in &plan.mutations {
        persist_mutation(pool, chat_id, message_id, &now, mutation, &id_map).await?;
    }
    Ok(plan.audit)
}
    ...
    match mutation {
        EntryMutation::Insert {
            ...
            if num_value.is_some() {
                sqlx::query(
                    "INSERT INTO chat_state_entries (chat_id, actor_id, kind, key, value, num_value, max_value, source_message_id, updated_at) VALUES (?1,?2,?3,?4,'',?5,?6,?7,?8)",
                )
                .bind(chat_id)
                .bind(actor_id)
                .bind(kind_str)
                ...
                sqlx::query(
                    "INSERT INTO chat_state_entries (chat_id, actor_id, kind, key, value, source_message_id, updated_at) VALUES (?1,?2,?3,?4,?5,?6,?7)",
                )
                .bind(chat_id)
                .bind(actor_id)
                .bind(kind_str)
        ...
        EntryMutation::UpdateNumeric {
            ...
            sqlx::query(
                "UPDATE chat_state_entries SET num_value=?1, max_value=?2, source_message_id=?3, updated_at=?4 WHERE id=?5",
            )
            .bind(num_value)
            .bind(max_value)
            .bind(source_id)
        ...
        EntryMutation::UpdateText { entry_id, value } => {
            sqlx::query(
                "UPDATE chat_state_entries SET value=?1, source_message_id=?2, updated_at=?3 WHERE id=?4",
            )
            .bind(value)
            .bind(source_id)
            .bind(now)
        ...
        EntryMutation::UpdateKind { entry_id, kind } => {
            sqlx::query("UPDATE chat_state_entries SET kind=?1, updated_at=?2 WHERE id=?3")
        ...
        EntryMutation::Delete { entry_id } => {
            sqlx::query("DELETE FROM chat_state_entries WHERE id = ?1")
                .bind(entry_id)
                .execute(pool)
                .await?;
        }
...
pub async fn revert_message_state_changes(
    ...
    for mutation in mutations {
        persist_revert(pool, chat_id, &now, &mutation).await?;
    }
    Ok(())
}
...
async fn persist_revert(
    ...
    match mutation {
        RevertMutation::RestoreNumeric {
            ...
            sqlx::query(
                "UPDATE chat_state_entries SET num_value=?1, source_message_id=-1, updated_at=?2 WHERE chat_id=?3 AND actor_id IS ?4 AND kind=?5 AND key=?6",
            )
            .bind(num_value)
            .bind(now)
            .bind(chat_id)
        ...
        RevertMutation::RestoreText {
            ...
            sqlx::query(
                "UPDATE chat_state_entries SET value=?1, source_message_id=-1, updated_at=?2 WHERE chat_id=?3 AND actor_id IS ?4 AND kind=?5 AND key=?6",
            )
            .bind(value)
            .bind(now)
            .bind(chat_id)
        ...
        RevertMutation::DeleteByKey {
            ...
            sqlx::query(
                "DELETE FROM chat_state_entries WHERE chat_id=?1 AND actor_id IS ?2 AND kind=?3 AND key=?4",
            )
            .bind(chat_id)
            .bind(actor_id)
            .bind(state_kind_str(*kind))
