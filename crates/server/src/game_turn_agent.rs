...
use crate::game_prompts::{build_inline_prose_agent_messages, build_structured_agent_messages};
use crate::game_tools::{
    handle_mechanical_tool_call, inline_prose_tool_specs, mechanical_tool_specs,
    parse_state_change_args, ToolSessionState,
};
        ...
        if result.tool_calls.is_empty() {
            if iteration == 0 {
                tracing::warn!(
                    game_id,
                    turn_id,
                    "mechanics tool loop: model returned no tool calls on first iteration"
                );
            }
            break;
        }
...
/// Full-structured engine mode turn:
/// 1. The model decides (from the PC's skills) whether dramatic checks are needed and rolls them.
/// 2. The model writes one streaming narration pass, calling scenario mechanic tools inline.
///    Each mechanic result is anchored into the prose with an inline marker so the UI can render
///    it as a block exactly where it fired. The model never resolves the PC's choices.
pub async fn run_tools_structured_phase(
    ...
    let mut session = ToolSessionState::new(game.clone(), detail.clone(), turn.clone());
    let mut messages =
